...
class CustomButton(Q.QPushButton):
    def __init__(self, text="", parent=None, is_mine=False, button_width=100, button_height=100):
    ...
    def reset(self):
        ...
        self.is_flagged = False
        self.setStyleSheet(self.styleSheet() + "background-color: lightgrey;")
    ...
    def mousePressEvent(self, event):
        ...
        elif event.button() == Qt.LeftButton:  # type: ignore
...
class MainWindow(Q.QMainWindow):
    ...
    def create_layout(self):
        ...
        self.label_title.setStyleSheet(
            "font-size: 40px; font-weight: bold; text-align: center; width: 100%;")
    ...
    def create_buttons(self, rows, cols, container):
        ...
        for i in range(rows):
            ...
            for j in range(cols):
                button = CustomButton(
                    f"{i}, {j}", self, button_width=button_width, button_height=button_height)
    ...
    def create_menu(self):
        menu = self.menuBar()
        diffiuculty_menu = menu.addMenu("&Difficulty")
        self.add_menu_item(diffiuculty_menu, "Easy", "Ctrl+E", self.EASY_MINE_COUNT, self.EASY_ROWS, self.EASY_COLS)
        self.add_menu_item(diffiuculty_menu, "Normal", "Ctrl+N", self.MED_MINE_COUNT, self.MED_ROWS, self.MED_COLS)
        self.add_menu_item(diffiuculty_menu, "Hard", "Ctrl+H", self.HARD_MINE_COUNT, self.HARD_ROWS, self.HARD_COLS)
 
    def add_menu_item(self, parent_menu, text, shortcut_text, mines, rows, cols):
        button_action = Q.QAction(text, self)
        button_action.setShortcut(QKeySequence(shortcut_text))
        parent_menu.addAction(button_action)
    ...
    def space_clicked(self, row, col, testing=False):
        ...
        if self.mine_list[row][col].is_mine:
            self.do_ending("You lost!", "Loser!")
 
            return  # do some game over game thing
            ...
            for i in range(-1, 2):
                ...
                for j in range(-1, 2):
                    ...
                    if (self.is_valid_index_in_mine_list(current_row, current_col) and 
                        ...
                        # print(f"looping {i}{j} : {current_row}{current_col}")
                        # print(self.mine_list[current_row][current_col])
 
        if self.click_count == (self.rows * self.cols) - self.total_mines:
            self.do_ending("Congragulations!", "Way to go, you did it!")
 
    def do_ending(self, title, message):
        self.reveal_all(True)
        end_screen = EndScreen(title=title, message=message, won=False)
