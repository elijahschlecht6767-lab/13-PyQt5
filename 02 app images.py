...
from PyQt5.QtGui import QIcon, QFont, QPixmap #qpixmap loads and manages the image file before it is placed in a label
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # x, y, width, height (typically) so placement, then size, horizontal then vertical
        ...
        # let's try putting greg in the bottem right cornet - we'll use MATH!
        self.piclabel.setGeometry(self.width(
        ) - self.piclabel.width(), self.height() - self.piclabel.height(), 300, 250)

        # % is mod, // is integer devision
        # let's try putting gred in the middle - we'll use MATH!
        self.piclabel.setGeometry((self.width(
